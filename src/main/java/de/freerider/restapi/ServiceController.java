...
class ServiceController implements ServiceAPI {
	...
	private final Person eric = new Person().setName("Eric", "Meyer").addContact("eric98@yahoo.com")
			.addContact("(030) 3945-642298");
	//
	private final Person anne = new Person().setName("Anne", "Bayer").addContact("anne24@yahoo.de")
			.addContact("(030) 3481-23352");
	//
	private final Person tim = new Person().setName("Tim", "Schulz-Mueller").addContact("tim2346@gmx.de");
	...
	private ArrayNode peopleAsJSON() {
		...
		people.forEach(c -> {
			...
			arrayNode.add(objectMapper.createObjectNode().put("name", c.lastName).put("first", c.firstName)
